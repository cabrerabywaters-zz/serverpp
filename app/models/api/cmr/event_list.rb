module Api
  ...
  class Cmr::EventList < WashOut::Type
    ...
    def self.fetch efi
      events = efi.events.includes(experience: :category).includes(:exchanges).order(:id).map { |e|
        {
          event_id: e.id, event_name: e.experience.name, ending_at: e.experience.ending_at,
          exchanges: Api::Cmr::Exchanges.fetch(e.exchanges), category: Api::Cmr::Category.fetch(e.experience.category),
          image_original_url: e.experience.image_url(:original), image_medium_url: e.experience.image_url(:medium),
          image_small_url: e.experience.image_url(:small), image_thumb_url: e.experience.image_url(:thumb)
        }
      }
      return { value: events }
    end
