class EcoBillingManager
  ...
  def income_by_experience
    ...
    income_by_experience = transactions.collect do |t|
      ...
      { experience: t["experience_name"], total_q: t["total_q"], price: t["price"], income: income, fee: fee, charge: charge, to_pay: (income - charge) }
    end
  end
  
  def transactions_grouped_by_experience
    experience_ids = Experience.where(state: 'on_sale').pluck(:id)
    Event.select("experience_id, experiences.name as experience_name, experiences.fee as experience_fee, experiences.amount as price, experiences.discounted_price as discounted_price, SUM(quantity) as total_q").
      where("experience_id IN (?)", experience_ids).
      where("events.created_at >= ?", invoice_start_at).
      where("events.created_at <= ?", invoice_end_at).
      joins(:experience).
      group("experience_id, experiences.name, experiences.fee, experiences.amount, experiences.discounted_price").
